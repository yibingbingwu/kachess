...
stmt :
     ...
     | add_stmt T_SEMICOLON
     | allocate_cursor_stmt
     | alter_table_stmt T_SEMICOLON
     | associate_locator_stmt
     | begin_transaction_stmt T_SEMICOLON
     | break_stmt
     | call_stmt
     | collect_stats_stmt
     | close_stmt
//     | cte_select_stmt
     ...
     | copy_stmt
     | commit_stmt T_SEMICOLON
     | create_database_stmt T_SEMICOLON
     | create_tmp_extnlfunc_stmt T_SEMICOLON
     | create_function_stmt
     | create_index_stmt T_SEMICOLON
     | create_local_temp_table_stmt T_SEMICOLON
     | create_package_stmt
     | create_package_body_stmt
     | create_procedure_stmt
     | create_table_stmt T_SEMICOLON
     | create_view_stmt T_SEMICOLON
     | declare_stmt
     | delete_stmt
     | describe_stmt T_SEMICOLON
     | drop_stmt T_SEMICOLON
     | end_transaction_stmt T_SEMICOLON
     | exec_stmt
     | exit_stmt
     | fetch_stmt
     | for_cursor_stmt
     ...
     | if_stmt
     | include_stmt
     | insert_table_stmt T_SEMICOLON
     | insert_directory_stmt
     | get_diag_stmt
     | grant_stmt
     | leave_stmt
     | map_object_stmt
     | merge_stmt T_SEMICOLON
     | open_stmt
     | print_stmt
     | quit_stmt T_SEMICOLON
     | raise_stmt
     | resignal_stmt
     | return_stmt
     | rollback_stmt T_SEMICOLON
     | select_stmt T_SEMICOLON?
     | show_tables_stmt T_SEMICOLON
     | signal_stmt
     | summary_stmt
     | update_stmt T_SEMICOLON
     | use_stmt
     | truncate_stmt T_SEMICOLON
     | values_into_stmt T_SEMICOLON
     | while_stmt
//     | label
//     | hive
//     | host
...
add_stmt :
       T_ADD2 (T_FILE | T_JAR) (T_S3)? ('-' | ident | '/' | '.' | L_INT )+
       ;

create_tmp_extnlfunc_stmt :
       T_CREATE T_TEMPORARY (T_FUNCTION ident T_AS L_S_STRING | T_MACRO ident create_routine_params expr)
       ;
...
assignment_stmt_single_item :
     ...
     | ident T_EQUAL '.' ident  // e.g. SET hive.output.file.extension = .csv
...
// TODO - Need to support or ignore:
// [SKEWED BY (col_name, col_name, ...)                  -- (Note: Available in Hive 0.10.0 and later)]
//     ON ((col_value, col_value, ...), (col_value, col_value, ...), ...)
//     [STORED AS DIRECTORIES]
//  [
create_table_stmt :
       T_CREATE (T_EXTERNAL | T_TEMPORARY)? T_TABLE (T_IF T_NOT T_EXISTS)? table_name create_table_preoptions?
       ( create_table_definition | create_table_like | create_table_as )
     ;

create_view_stmt :
        T_CREATE T_VIEW (T_IF T_NOT T_EXISTS)? table_name T_AS select_stmt
     ;

create_table_like :
        T_LIKE table_name create_table_options?
        ;

create_table_as :
        create_table_options? T_AS select_stmt
        ;
...
table_partition_clause :
        T_PARTITIONED T_BY T_OPEN_P create_table_columns T_CLOSE_P
        ;

table_row_fmt_clause :
          (T_ROW T_FORMAT row_format )? T_STORED T_AS file_format
        | (T_ROW T_FORMAT row_format )
        ;

row_format :
        ( T_DELIMITED create_table_hive_row_format_fields* )?
        ( T_SERDE L_S_STRING (T_WITH T_SERDEPROPERTIES T_OPEN_P assign_single_name_key (T_COMMA assign_single_name_key)* T_CLOSE_P)? )?
        ( T_STORED T_AS file_format )?
        ;

assign_single_name_key :
        (L_S_STRING | L_D_STRING) T_EQUAL (L_S_STRING | L_D_STRING)
        ;
...
file_format :
        ...
        | T_TEXTFILE
        | T_PARQUET
        | T_INPUTFORMAT L_S_STRING T_OUTPUTFORMAT L_S_STRING
        ;


table_clustered_clause :
        T_CLUSTERED T_BY T_OPEN_P column_name (T_COMMA column_name)* T_CLOSE_P
        (T_SORTED T_BY T_OPEN_P column_name (T_COMMA column_name)* T_CLOSE_P)?
        T_INTO L_INT T_BUCKETS
        ;

table_data_loc_clause :
        T_LOCATION L_S_STRING
        ;
...
create_table_columns_item :
        column_name ( dtype dtype_len? | complex_dtypes ) dtype_attr*
          create_table_column_inline_cons* ( T_COMMENT ( L_S_STRING | L_D_STRING ) )?
     | (T_CONSTRAINT ident)? create_table_column_cons
     ;

column_name :
       ident
     | T_BACKTICK ident T_BACKTICK
     ;
...
table_properties_clause :
        T_TBLPROPERTIES T_OPEN_P (L_S_STRING | L_D_STRING) (T_EQUAL|T_COLON) (L_S_STRING | L_D_STRING)
                (T_COMMA (L_S_STRING | L_D_STRING) (T_EQUAL|T_COLON) (L_S_STRING | L_D_STRING))* T_CLOSE_P
     ;
...
alter_table_stmt :
       T_ALTER T_TABLE table_name ( alter_table_item | alter_table_partition | alter_table_location |
       alter_table_properties )
     ;

alter_table_partition :
        ( T_ADD2 (T_IF T_NOT T_EXISTS)? | T_DROP (T_IF T_EXISTS)? ) partition_spec table_data_loc_clause?
      ;

alter_table_location :
        T_SET table_data_loc_clause
      ;
...
alter_table_properties :
        T_SET T_TBLPROPERTIES T_OPEN_P L_S_STRING T_EQUAL L_S_STRING T_CLOSE_P
      ;
...
array_dtypes :
    T_ARRAY T_LESS (dtype dtype_len? | complex_dtypes) T_GREATER
    | (T_ROW | T_ARRAY) T_OPEN_P ident? dtype (T_COMMA ident? dtype)* T_CLOSE_P  // The Presto definition
    ;

struct_dtypes :
    T_STRUCT T_LESS label  (dtype dtype_len? | complex_dtypes)
        (T_COMMA label  (dtype dtype_len? | complex_dtypes))* T_GREATER
    ;

map_dtypes :
    T_MAP T_LESS dtype dtype_len? T_COMMA (dtype dtype_len? | complex_dtypes) T_GREATER
    ;
...
drop_view :
       T_DROP T_VIEW (T_IF T_EXISTS)? table_name
      ;
...
insert_table_stmt :
      cte_select_stmt? ( insert_stmt | multitable_insert_stmt )
      ;

multitable_insert_stmt :
       from_clause insert_stmt (insert_stmt)*
       ;

insert_stmt :           // INSERT statement
       T_INSERT (T_OVERWRITE T_TABLE | T_INTO T_TABLE?) table_name partition_spec? insert_stmt_cols? (select_stmt | insert_stmt_rows) distribute_clause?
     ;

partition_spec:
       T_PARTITION T_OPEN_P column_name (bool_expr_binary_operator expr)? (T_COMMA column_name (bool_expr_binary_operator expr)?)* T_CLOSE_P
     ;
...
// TODO - the parenthesises is a trouble ... Should apply at lower level. This is just
// a hack, assuming Hive engine already vetted it and the usage is rather limited:
distribute_clause :
       (T_DISTRIBUTE|T_CLUSTER) T_BY T_OPEN_P? column_name (T_COMMA column_name)* T_CLOSE_P?
      ;
...
cte_select_stmt_item :
       ident cte_select_cols? T_AS T_OPEN_P select_stmt T_CLOSE_P
     ;
...
fullselect_stmt_item :
       basic_select_stmt
     | composite_select_stmt
     | T_OPEN_P fullselect_stmt T_CLOSE_P
     ;

fullselect_set_clause :
       T_UNION (T_ALL | T_DISTINCT)?
     | T_EXCEPT T_ALL?
     | T_INTERSECT T_ALL?
     ;

composite_select_stmt :
       T_OPEN_P fullselect_stmt T_CLOSE_P (T_AS? ident)? from_join_clause
     ;

basic_select_stmt :
       (T_SELECT | T_SEL) select_list into_clause? from_clause? where_clause? group_by_clause?
       (having_clause | qualify_clause)?
       distribute_clause? order_by_clause? window_as_options? select_options?
     ;

window_as_options :
        T_WINDOW ident T_AS expr_func_over_window_clause (T_COMMA ident T_AS expr_func_over_window_clause)*
     ;
...
select_list_item :
         select_list_no_asterisk
       | select_list_asterisk
     ;

select_list_no_asterisk :
        ( expr | bool_expr ) select_list_alias?
     ;

timezone_expr :
        T_AT T_TIME T_ZONE L_S_STRING
     ;

select_list_alias :
       {!_input.LT(1).getText().equalsIgnoreCase("INTO") && !_input.LT(1).getText().equalsIgnoreCase("FROM")}?
       T_AS? ( ident | L_D_STRING )
     ;
...
from_clause :
       T_FROM from_table_clause (lateral_view_clause)* (from_join_clause)*
     ;

lateral_view_clause :
       T_LATERAL T_VIEW T_OUTER? expr_func lv_alias T_AS lv_columns
     ;
...
from_table_name_clause :
       table_name table_sample_clause? from_alias_clause?
     ;

table_sample_clause :
       T_TABLESAMPLE (ident | T_SYSTEM)? T_OPEN_P
       ( T_BUCKET L_INT T_OUT T_OF L_INT (T_ON column_name)? | L_DEC T_PERCENT? | L_INT T_ROWS? | human_readable_size )
       T_CLOSE_P
     ;

// Having trouble fitting in B, K, M, G into this rule: rule placement order matters: only if placed before ID
// But then breaks some other rules.
human_readable_size :
       L_INT L_ID?
     ;

from_subselect_clause :
       T_OPEN_P from_subselect_clause T_CLOSE_P
     | T_OPEN_P select_stmt T_CLOSE_P from_alias_clause?
     ;

from_join_clause :
       T_COMMA from_table_clause
     | from_cross_join_clause
     | from_join_type_clause from_table_clause T_ON bool_expr
     ;

from_cross_join_clause :
       T_CROSS T_JOIN expr (T_AS? ident (T_OPEN_P column_name (T_COMMA column_name)* T_CLOSE_P)?)?
     | from_join_type_clause from_table_clause
     ;
...
from_alias_clause :
      { !_input.LT(1).getText().equalsIgnoreCase("GROUP") }? T_AS? ident (T_OPEN_P ident (T_COMMA ident)* T_CLOSE_P)?
     ;

table_name :
       ident
     | T_OPEN_P ident T_CLOSE_P
     | T_BACKTICK ident T_BACKTICK
     ;
...
group_by_clause :
       T_GROUP T_BY (group_by_columns)? group_set_clause? with_rollup_clause?
     ;

group_by_columns :
     {!_input.LT(1).getText().equalsIgnoreCase("GROUPING")}? (expr | bool_expr) (T_COMMA (expr | bool_expr))*
     ;

group_set_clause :
      T_GROUPING T_SETS T_OPEN_P group_set_items T_CLOSE_P
      ;

group_set_items :
         group_set_item (T_COMMA group_set_item)*
       ;

group_set_item :
      T_OPEN_P T_CLOSE_P
      | T_OPEN_P? (expr | bool_expr) T_CLOSE_P?
      | T_OPEN_P (expr | bool_expr) (T_COMMA (expr| bool_expr))+ T_CLOSE_P
     ;

with_rollup_clause :
        T_WITH (T_CUBE | T_ROLLUP)
     ;
...
order_by_clause :
       ( T_ORDER | T_SORT ) T_BY expr (T_ASC | T_DESC)? (T_COMMA expr (T_ASC | T_DESC)?)*
     ;
...
bool_expr_unary :
    ...
    | expr T_NOT? T_BETWEEN expr T_AND expr
...
expr_map_array :
     T_OPEN_SB ( expr | non_reserved_words ) T_CLOSE_SB  // Array, Map, etc. expression
     ;
expr_array_init :
     T_ARRAY T_OPEN_SB (expr (T_COMMA expr)*)? T_CLOSE_SB
     ;

expr_lambda :
     tuple_def T_SUB T_GREATER expr
     ;

expr_presto_induced :
       T_POSITION T_OPEN_P expr T_IN expr T_CLOSE_P
     | T_EXTRACT T_OPEN_P time_const T_FROM expr T_CLOSE_P
     ;

time_const:
       T_HOUR | T_DAY | T_MINUTE | T_SECOND
     ;

expr_atom :
       ident    # colNameInExpr
     | expr_atom_constants  # constInExpr
     ;
...
expr_case_simple :
       T_CASE expr (T_WHEN expr T_THEN (expr|bool_expr))+ (T_ELSE (expr|bool_expr))? T_END
     ;

expr_case_searched :
       T_CASE (T_WHEN bool_expr T_THEN (expr|bool_expr))+ (T_ELSE (expr|bool_expr))? T_END
     ;

expr_cast:
       (T_CAST | T_TRY_CAST) T_OPEN_P (expr | bool_expr) T_AS (dtype dtype_len? | complex_dtypes) T_CLOSE_P
     ;
...
expr_agg_window_func :
     ...
     | T_COUNT T_OPEN_P ((expr_func_all_distinct? expr) | expr_func_param_any) T_CLOSE_P expr_func_over_clause?
     | T_COUNT_BIG T_OPEN_P ((expr_func_all_distinct? expr) | expr_func_param_any) T_CLOSE_P expr_func_over_clause?
     ...
     | ident T_OPEN_P expr_func_all_distinct? ((expr (T_COMMA expr)*) | expr_func_param_any)? T_CLOSE_P expr_func_over_clause?
     ;

expr_func_param_any: T_MUL
     ;
...
expr_func_over_clause :
       T_OVER expr_func_over_window_clause
     | T_OVER ident
     ;

expr_func_over_window_clause :
       T_OPEN_P expr_func_partition_by_clause? order_by_clause? expr_window_spec? T_CLOSE_P
     ;
...
expr_window_spec:
      (T_ROWS | T_RANGE) (T_UNBOUNDED | L_INT ) T_PRECEDING
    | (T_ROWS | T_RANGE) T_BETWEEN (T_UNBOUNDED | L_INT | expr) T_PRECEDING T_AND ( L_INT T_PRECEDING | T_CURRENT T_ROW | (T_UNBOUNDED | L_INT) T_FOLLOWING )
    | (T_ROWS | T_RANGE) T_BETWEEN T_CURRENT T_ROW T_AND ( T_CURRENT T_ROW | (T_UNBOUNDED | L_INT) T_FOLLOWING )
    | (T_ROWS | T_RANGE) T_BETWEEN L_INT T_FOLLOWING T_AND ( L_INT | T_UNBOUNDED ) T_FOLLOWING
    ;

expr_spec_func :
       T_ACTIVITY_COUNT
     | (T_CAST | T_TRY_CAST) T_OPEN_P expr T_AS  dtype dtype_len? T_CLOSE_P
...
expr_complex_init:
       (T_ARRAY | T_MAP) T_OPEN_P expr_func_params? T_CLOSE_P
     ;
...
func_param :
       {!_input.LT(1).getText().equalsIgnoreCase("INTO")}? (ident T_EQUAL T_GREATER?)? ( expr | bool_expr )
     ;
...
ident :
       (L_ID | non_reserved_words) ('.' (L_ID | non_reserved_words))* (T_OPEN_SB (L_S_STRING | L_INT | L_ID) T_CLOSE_SB)*
     | L_B_STRING
     | ident '.' ident
     ;
...
//     | T_FULL
     ...
     | T_GROUP
     | T_GROUPING
//     | T_GROUPING_ID    // Doesnt work for Hive, sorry Presto
     | T_HANDLER
     | T_HASH
     | T_HAVING
     | T_HDFS
...
T_GROUPING_ID     : G R O U P I N G '_' '_' I D ;
...
// See https://stackoverflow.com/questions/53465057/ what an idea that was!
L_S_STRING  : '\'' ( ('\'\'') | ('\\'+ ~'\\') | ~('\'' | '\\') )* '\'' ;

L_D_STRING  : '"' (L_STR_ESC_D | .)*? '"'                              // Double quoted string literal
            ;
L_B_STRING  : '`' (~('`'))* '`'                                        // Backtick quoted string literal
            ;
L_INT       : L_DIGIT+ ('L')?
            | L_DIGIT+ E L_DIGIT+ ;                                    // Integer
L_DEC       : L_DIGIT+ '.' ~'.' L_DIGIT* (E L_DIGIT+)? ('F')?          // Decimal number
            | '.' L_DIGIT+ ('F')?
            ;
...
L_FILE      : (L_ALPHA ':' '\\'?)? L_ID ('\\' L_ID)*                   // File path (a/b/c Linux path causes conflicts with division operator and handled at parser level)
            ;

L_LABEL     : (L_ALPHA | L_DIGIT | '_' | '`')* ':'
            ;


fragment
L_ID_PART  :
            (L_ALPHANUMERIC | '_')* (L_ALPHA | '_')+ (L_ALPHANUMERIC | '_')*    // Identifier part
            | '$' '{' .*? '}'
...
fragment L_ALPHA : [a-zA-Z];
fragment L_DIGIT : [0-9];
fragment L_ALPHANUMERIC: L_ALPHA | L_DIGIT;
fragment L_BLANK : (' ' | '\t' | '\r' | '\n') ;
